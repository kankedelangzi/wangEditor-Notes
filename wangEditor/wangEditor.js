...
(function(global, factory) {// 完成模块化，并且是兼容的写法  如果支持exports则用module.exports导出
	...
	*/
	...
	function createElemByHTML(html) {
		...
		div.innerHTML = html;// 先将要创建的节点赋值给临时节点div
		return div.children;// 然后以节点的形式将其返回。很妙，避免了逐个创建
	}

	// 是否是 DOM List
	function isDOMList(selector) {// 判断是否为一个节点的集合，，注意了，如果是一个节点，这里是返回否的
		// 但是如果是[ 一个节点 ]  这种形式的返回的是true
		if (!selector) {// 没有传入返回否
			return false;
		}
		if (selector instanceof HTMLCollection || selector instanceof NodeList) {
			return true;// 传入的是节点集合或者是html集合
		}
		return false;
	}
	...
	function DomElement(selector) {
		...
		this.length = length;
		/*
			通过for循环之后，在创建此对象时就会变成一个类数组对象
			{
			  “0” : <div></div>,
			  "1": <div></div>,
			  length: 2
			}
			其实是借鉴了jquery的做法，这样既可以用for循环进行遍历，又可以挂在类似selector，length等属性和
			方法。
		 */
	}
	...
	DomElement.prototype = {
		constructor: DomElement,// 将构造函数定义为 DomElement防止指向错误

		// 类数组，forEach
		forEach: function forEach(fn) {
			// 数组是自己具备forEach方法的，但是通过DomElement方法创建的类数组对象却没有这个遍历方法，这里手动封装一个
			// 便于后边的遍历操作，但是这里定义的不是很好，因为数组原生的回调里边是（currentValue， index, array）
			// 但是这里的回调里是fn.call(elem, elem, i);有一定的差别
			...
			for (i = 0; i < this.length; i++) {
				...
				if (result === false) {
					// 这样就可以在调用时如果返回了false就终止循环，这是数组原生的forEach做不到的
					break;
				}
			}
			return this;
		...
		// 获取第几个元素
		get: function get(index) {// 获取元素中的第几个
			var length = this.length;
			if (index >= length) {
				// 当传入的值大于总长度时比如   length是4    传入了  index是7
				index = index % length;// 通过求余运算换算成3 进行返回，虽然不会出错，但是这样感觉并不合理
				// 最好是抛出错误。不过基于此功能是内部使用，不抛出异常也是情有可原的。
			}
			return $(this[index]);
		},
		...
		on: function on(type, selector, fn) {
			...
			// 简单的事件绑定 但是兼容了事件代理的模式
			// 比如： div.on('click mouseenter', 'span', function(){})
			// 这就是做了事件代理，
			if (!fn) {
				// 处理不做代理的情况
		...
		addClass: function addClass(className) {
			...
			return this.forEach(function(elem) {
				...
				if (elem.className) {
					...
					// 添加 class
					if (arr.indexOf(className) < 0) {// 防止重复添加
						arr.push(className);
					}
					// 修改 elem.class
					elem.className = arr.join(' ');// 转为字符串之后赋值给当前元素
				} else {
					elem.className = className;// 如果元素之前不存在className那么直接赋值
				}
			});
		},
		...
		removeClass: function removeClass(className) {
			if (!className) {// 验证参数的传入情况
				return this;// 这种return this 是为了链式操作
			}
			return this.forEach(function(elem) {
				var arr = void 0;
				if (elem.className) {
					// 解析当前 className 转换为数组
					arr = elem.className.split(/\s/);
					arr = arr.filter(function(item) {//通过过滤器直接过滤掉要删除的类名
						item = item.trim();
						// 删除 class
						if (!item || item === className) {// 这是去掉这个类名的操作所在
							return false;
						}
						return true;
					});
		...
		css: function css(key, val) {
			...
			// 这样currentStyle 是 “position:absolute;”

			return this.forEach(function(elem) {
				// 获取当前元素的style属性以<div style="width:100px;height:200px;"></div>
				...
				if (style) {
					...
					// styleArr ["width:100px","width:100px"]
					...
					// 执行结束 resultArr = ["width:100px”,“height:200px"];
					...
					// 结果
					elem.setAttribute('style', resultArr.join('; '));// 最后将结果合并为字符串并赋值
				} else {
					// style 无值
					elem.setAttribute('style', currentStyle);// 如果元素不存在style标签，那么可以直接赋值
				}
			});
		},
		...
		children: function children() {
			var elem = this[0];// 获取集合中的第一个元素
			if (!elem) {// 如果一个也不存在那么直接返回空
				return null;
			}
		...
		remove: function remove() {
			return this.forEach(function(elem) {
				if (elem.remove) {
					elem.remove();// 如果传入的不是原生的节点那么就进行递归
				} else {
					var parent = elem.parentElement;
					parent && parent.removeChild(elem);// 这是调用原生的删除节点的操作
				}
			});
		},
		...
		isContain: function isContain($child) {
			var elem = this[0];
			var child = $child[0];
			return elem.contains(child);// 这个地方的contains方法貌似是老的ie浏览器才有的
			// 意思是a元素中 contains 包含b元素
		},
		...
		text: function text(val) {
			if (!val) {
				...
				return elem.innerHTML.replace(/<.*?>/g, function() {
					// 这里由于text方法本身存在兼容性所以直接使用innerHtml方法将<> 替换掉这样就能获取到无标签的text了
					// 这里注意这种替换方式
					return '';
				});
		...
		parentUntil: function parentUntil(selector, _currentElem) {
			...
			var parent = elem.parentElement;// ie的老的写法获取父节点
			var i = void 0;
			for (i = 0; i < length; i++) {
				if (parent === results[i]) {
					// 找到，并返回
		...
		equal: function equal($elem) {
			if ($elem.nodeType === 1) {// 判断如果是元素节点那么
				return this[0] === $elem;
			} else {//非元素节点，那么就认为这是一个集合。这种方式貌似并不是很好
		...
		insertBefore: function insertBefore(selector) {
			// 这个操作是将当前对象中选取的元素插入到    selector这个元素的前边，这点要弄清
			var $referenceNode = $(selector);
			var referenceNode = $referenceNode[0];
			if (!referenceNode) {// 如果selector这个节点不存在那么就返回this继续向下执行，
				// 这一点也不是很好，应该抛出错误
				return this;
			}
			return this.forEach(function(elem) {
	...
	*/
	...
	// 替换 html 特殊字符
	function replaceHtmlSymbol(html) {// 将 <> “” 转译一下
		if (html == null) {
			return '';
		}
		// 思考一下为什么下边可以用链式操作？
	...
	*/
	...
	Bold.prototype = {
		...
		onClick: function onClick(e) {
			...
			// 执行 bold 命令
			editor.cmd.do('bold');// 进入comond  实则通过document.execCommand('bold')进行设置的
		...
		tryChangeActive: function tryChangeActive(e) {
			...
			if (editor.cmd.queryCommandState('bold')) {
				this._active = true;
				$elem.addClass('w-e-active');// 可见是加载在menu的容器元素上的类名，那么这个类名影响的是在头部工具栏的按钮
				// 的状态也就是当激活状态时，既当前已经加粗，那么  B   这个按钮就是一个active的状态，这个做法很类似word等编辑器的做法
			} else {
				this._active = false;
	...
	/*
	    droplist
	*/
	var _emptyFn = function _emptyFn() {};// 定义了一个空函数
	...
	*/
	...
	Head.prototype = {
		...
		// 执行命令  这里的参数value就是要添加的标签
		_command: function _command(value) {// 塞入一个块级标签   比如 H1   H2 等，会根据value的传值进行添加标签
			var editor = this.editor;
			editor.cmd.do('formatBlock', value);
		},

		// 试图改变 active 状态
		tryChangeActive: function tryChangeActive(e) {// 激活当行栏的标签
			var editor = this.editor;
			var $elem = this.$elem;
			var reg = /^h/i;
		...
		_createPanel: function _createPanel(text, link) {
			...
			var panel = new Panel(this, {
				...
				tabs: [{
							...
							title: '链接',
							// 模板
							tpl: '<div>\n                            <input id="' + inputTextId + '" type="text" class="block" value="' + text +
								'" placeholder="\u94FE\u63A5\u6587\u5B57"/></td>\n                            <input id="' + inputLinkId +
								'" type="text" class="block" value="' + link +
								'" placeholder="http://..."/></td>\n                            <div class="w-e-button-container">\n                                <button id="' +
								btnOkId + '" class="right">\u63D2\u5165</button>\n                                <button id="' + btnDelId +
								'" class="gray right" style="display:' + delBtnDisplay +
								'">\u5220\u9664\u94FE\u63A5</button>\n                            </div>\n                        </div>',
							// 事件绑定
							events: [
								// 插入链接
		...
		_createPanel: function _createPanel(value) {
			...
			var panel = new Panel(this, {
				...
				tabs: [{
							...
							title: '插入代码',
							// 模板
							tpl: '<div>\n                        <textarea id="' + textId + '" style="height:145px;;">' + value +
								'</textarea>\n                        <div class="w-e-button-container">\n                            <button id="' +
								btnId + '" class="right">\u63D2\u5165</button>\n                        </div>\n                    <div>',
							// 事件绑定
							events: [
								// 插入代码
								{
			...
			var panel = new Panel(this, {
				...
				tabs: [{
							...
							tpl: '<div>\n                        <p style="text-align:left; padding:5px 0;">\n                            \u521B\u5EFA\n                            <input id="' +
								...
								textColNum +
								'" type="text" value="5" style="width:40px;text-align:center;"/>\n                            \u5217\u7684\u8868\u683C\n                        </p>\n                        <div class="w-e-button-container">\n                            <button id="' +
								btnInsertId + '" class="right">\u63D2\u5165</button>\n                        </div>\n                    </div>',
							// 事件绑定
							events: [{
								// 点击按钮，插入表格
			...
			var panel = new Panel(this, {
				...
				tabs: [{
					...
					tpl: '<div>\n                        <div class="w-e-button-container" style="border-bottom:1px solid #f1f1f1;padding-bottom:5px;margin-bottom:5px;">\n                            <button id="' +
						addRowBtnId + '" class="left">\u589E\u52A0\u884C</button>\n                            <button id="' + delRowBtnId +
						'" class="red left">\u5220\u9664\u884C</button>\n                            <button id="' + addColBtnId +
						'" class="left">\u589E\u52A0\u5217</button>\n                            <button id="' + delColBtnId +
		...
		_createEditPanel: function _createEditPanel() {
			...
			var tabsConfig = [{
				...
				tpl: '<div>\n                    <div class="w-e-button-container" style="border-bottom:1px solid #f1f1f1;padding-bottom:5px;margin-bottom:5px;">\n                        <span style="float:left;font-size:14px;margin:4px 5px 0 5px;color:#333;">\u6700\u5927\u5BBD\u5EA6\uFF1A</span>\n                        <button id="' +
					width30 + '" class="left">30%</button>\n                        <button id="' + width50 +
					'" class="left">50%</button>\n                        <button id="' + width100 +
					'" class="left">100%</button>\n                    </div>\n                    <div class="w-e-button-container">\n                        <button id="' +
					delBtn + '" class="gray left">\u5220\u9664\u56FE\u7247</button>\n                    </dv>\n                </div>',
				events: [{
					selector: '#' + width30,
					type: 'click',
					fn: function fn() {
		...
		_createInsertPanel: function _createInsertPanel() {
			...
			var tabsConfig = [{
					...
					tpl: '<div class="w-e-up-img-container">\n                    <div id="' + upTriggerId +
						...
						upFileId +
						'" type="file" multiple="multiple" accept="image/jpg,image/jpeg,image/png,image/bmp,image/tiff"/>\n                    </div>\n                </div>',
					events: [{
						// 触发选择图片
				...
				}, // first tab end
				{
					...
					tpl: '<div>\n                    <input id="' + linkUrlId +
						...
						linkBtnId + '" class="right">\u63D2\u5165</button>\n                    </div>\n                </div>',
					events: [{
						selector: '#' + linkBtnId,
						type: 'click',
						fn: function fn() {
	...
	*/
	...
	Menus.prototype = {
		...
		_addToToolbar: function _addToToolbar() {
			var editor = this.editor;
			var $toolbarElem = editor.$toolbarElem;// 导航栏
			var menus = this.menus;// 菜单
			objForEach(menus, function(key, menu) {
				// 每一个menu都有一个自己设置的容器盒子存放在$elem这个属性中，想在将其添加到头部的工具栏中
				var $elem = menu.$elem;
				if ($elem) {
	...
	*/
	...
	Text.prototype = {
		...
		// 获取 设置 text
		text: function text(val) {// text方法和html区别就是text方法在外层添加了一个p标签
			var editor = this.editor;
			var $textElem = editor.$textElem;
			if (val == null) {
		...
		_clearHandle: function _clearHandle() {
			...
			$textElem.on('keydown', function(e) {
				if (e.keyCode !== 8) {// 如果不是删除键那不做监听
					return;
				}
				var txtHtml = $textElem.html().toLowerCase().trim();
				if (txtHtml === '<p><br></p>') {// 删除到最后一行时阻止默认行为
...
/*
	当一个HTML文档切换到设计模式 designMode时，文档对象暴露 execCommand 方法，
	该方法允许运行命令来操纵可编辑区域的内容。大多数命令影响文档的选择（粗体，斜体等），
	而其他命令插入新元素（添加链接）或影响整行（缩进）。当使用contentEditable时，调用 execCommand() 将影响当前活动的可编辑元素。
 */
	...
	*/
	...
	Command.prototype = {
		...
		_insertHTML: function _insertHTML(html) {
			...
			if (this.queryCommandSupported('insertHTML')) {//如果浏览器支持编辑指令insertHTML那么就用浏览器默认的
				// W3C
		...
		// 封装 execCommand
		_execCommand: function _execCommand(name, value) {// 就是对document.execCommand的一层封装
		...
		/*
			document.queryCommandState   会检查 当前光标所在地方是否有某种样式
			例如：document.queryCommandState('italic');
	...
	*/
	...
	API.prototype = {
		...
		collapseRange: function collapseRange(toStart) {
			...
			if (range) {
				range.collapse(toStart);// 将光标定位到选区开始位置。
			}
		},
		...
		getSelectionText: function getSelectionText() {
			var range = this._currentRange;
			if (range) {
				return this._currentRange.toString();// 调用toString方法转为字符串并返回
			} else {
				return '';
			}
		},
	...
	*/
	...
	UploadImg.prototype = {
		...
		_alert: function _alert(alertInfo, debugInfo) {
			var editor = this.editor;
			var debug = editor.config.debug;// 如果允许错误调试模式
		...
		// 根据链接插入图片
		insertLinkImg: function insertLinkImg(link) {// 插入图片时说明已经上传成功了
			// link是上传的url
			var _this2 = this;
			...
			img.onerror = function() {
				img = null;
				// 无法成功下载图片
				_this2._alert('插入图片错误', 'wangEditor: \u63D2\u5165\u56FE\u7247\u51FA\u9519\uFF0C\u56FE\u7247\u94FE\u63A5\u662F "' + link +
					'"\uFF0C\u4E0B\u8F7D\u8BE5\u94FE\u63A5\u5931\u8D25');
				return;
			};
		...
		uploadImg: function uploadImg(files) {
			...
			arrForEach(files, function(file) {
				var name = file.name;
				var size = file.size;
				if (/\.(jpg|jpeg|png|bmp|tiff)$/i.test(name) === false) {
					// 后缀名不合法，不是图片
			...
			if (uploadImgServer && typeof uploadImgServer === 'string') {
				...
				objForEach(uploadImgParams, function(key, val) {// 遍历参数列表，将参数添加到url和formdata对象中
	...
	*/
	...
	Editor.prototype = {
		...
		_initDom: function _initDom() {
			...
			var toolbarSelector = this.toolbarSelector;// 这是编辑器的id
			var $toolbarSelector = $(toolbarSelector);// 变成一个DomElement对象
			...
			// 定义变量
			var $toolbarElem = void 0, // 头部工具栏
				$textContainerElem = void 0,// 编辑体
				$textElem = void 0,
				$children = void 0;
			...
			// 编辑区域
			$textElem = $('<div></div>');// 编辑的区域
			// contenteditable 这个是富文本编辑器的根本属性，也是本组件的核心技术。请goole之
		...
		_initSelectionAPI: function _initSelectionAPI() {
			this.selection = new API(this);// 3455行左右
		},
		...
		initSelection: function initSelection() {
			...
			// 将选区定位到最后一个元素
			// 通过ture这个参数使得光标移动到选区最后也就是整个编辑区域的最后
		...
		_bindEvent: function _bindEvent() {
			...
			if (onchange && typeof onchange === 'function') {// 如果传入了onchange的监听函数
				// 触发 change 的有三个场景：
				...
				this.change = function() {
					...
					if (currentHtml.length === beforeChangeHtml.length) {// 如果当前的html信息和只爱去哪的一致
						return;
					}

					// 执行，使用节流
					if (onChangeTimeoutId) {// 如果onChangeTimeoutId不是0 说明说明当前有在执行的change事件
						clearTimeout(onChangeTimeoutId);// 取消这个事件
					}
					onChangeTimeoutId = setTimeout(function() {
						// 放在一个setTimeout函数里边避免过于频繁的触发
						// 触发配置的 onchange 函数
						onchange(currentHtml);// 执行onchange的监听事件，并且传入currentHtml作为参数，这个很实用
						beforeChangeHtml = currentHtml;// 每次执行之后都要往上推一下html内容
					}, 200);
				};
			}
		},
		...
		create: function create() {
			...
			// 封装 command API 实则封装了 document.execCommand命令，并且在ie的兼容性方面做了一些进一步封装
			this._initCommand();

			// 封装 selection range API
			this._initSelectionAPI();// 选区功能

			// 添加 text
			this._initText();// 文本功能

			// 初始化菜单
			this._initMenus();// 工具菜单功能
